...
**③ 产品功能 / UI 更新时，固定检查四处面向用户的说明。**
每次更新产品，默认逐一检查并按需同步：① 产品内「新手指引」；② 产品内「Agent 指引」；
③ 主仓库 `DrinkTea905/paper-piggy` 的 GitHub README / 项目介绍；
...
### 6.1 2026-07-22：Codex 逐文件 Git 扫描引发 WMI 正反馈雪崩

**事故表现**：`WmiPrvSE.exe` 持续占用 2.2～2.9 个 CPU 核，`winmgmt` 同时高负载，超过 10 分钟不收敛。

**根因链路**：仓库当时有 738 个未跟踪文件；Codex 桌面端对每个文件单独 spawn `git hash-object` 和 `git diff --no-index`，没有批处理、数量上限或对 `site-packages`/构建产物目录的启发式跳过。调用带 `killProcessTree:true` 和超时，OneDrive 路径与过高并发造成大量超时；每次超时又执行 `taskkill.exe /pid <PID> /t /f`，实测约 9 次/秒。`taskkill /t` 通过 WMI 全量枚举 `Win32_Process`，把 `WmiPrvSE` 推到 221%～290%；系统变慢又制造更多 Git 超时，形成永不收敛的正反馈。

**附带危险**：高峰期系统每秒创建/销毁约 18 个进程，陈旧 PID 会快速回收。174 个 kill 目标中只有 10 个当时仍是 git，27 个已经回收成 `taskkill` 自身，其余约 137 个在被杀时已不存在。`/f` 强杀、`/t` 连子树和陈旧 PID 叠加，存在误杀任意无关进程的风险。
...
- **来源权重校准（API 模式已完成；本地模式仍需独立金标）**：v1.0.29 用38条真实查询完成
  25条调参 + 13条盲测，API reranker 的来源评价加成从 0.50 校准为 0.30；前三命中率和前八召回率
  在盲测均保持100%，nDCG 优于旧值。候选快照与离线校准器在 `src/tests/retrieval_*`。
  本地 ONNX 是 0~10+ 的原始 logit，不能套用 API 的 0~1 结论，暂时保留 0.50；以后若要调整，
  必须另采本地模式金标，不得凭感觉改。
