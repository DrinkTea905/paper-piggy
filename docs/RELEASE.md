...
⚠️ **又踩过（2026-07-22）**：Codex 的 `shell_environment_policy.include_only` 如果漏掉
`LOCALAPPDATA`，旧脚本会把用户级候选错误拼成当前目录下的 `Programs\...`，明明已安装也误报
“没找到 ISCC”。现在构建脚本会在变量缺失时由 `USERPROFILE\AppData\Local` 回退解析；用户级
Codex 配置仍应保留 `APPDATA` 与 `LOCALAPPDATA`，否则 GitHub CLI 也会找不到原有登录配置。
...
- [ ] `python-docx` 已装（用于 DOCX 全文解析与深索）
