...
PDF 状态分为 `ocr_pending / ocr_failed / missing_pdf / invalid_pdf / ok_native / ok_ocr`；其他全文格式使用 `missing_file / invalid_file / ok_text`。
...
- 取消：`POST /build/cancel` → 关闭由应用持有的构建进程树。Windows 必须用 Job Object 管理并终止成员进程，禁用会经 WMI 全量枚举且可能误杀回收 PID 的 `taskkill /T /F`；其他平台保留各自的进程组实现。

### 自动更新

`_auto_update_loop`（`server.py:202`，startup 起的 daemon 线程，`server.py:364`）：
按 `settings.auto_update`（默认每天 07:00）比对 `_source_signature()`（Zotero 走 sqlite+**-wal** 的 mtime，folder 走受支持全文文件数量+最新 mtime）。
...
- `SCHEMA_VERSION` 的当前值以 `wiki_store.py` 为准。`WIKI.md`（= `WIKI_SCHEMA_MD`，`config.py:97`）会被 MCP `initialize` **整篇下发给 agent**（`mcp_server.py:90-107`、`:174-177`），所以它过期 = agent 照旧规约干活。
...
0_Agent资料库/          # AGENT_RELY_NAME，config.py:84
  ...
  记忆/  项目记忆.md（当前真相，≤200 行）、工具经验.md（题目无关的作业经验）、
         主题档案/<主题名>.md（一主题一档案，首部带状态行）、变更日志.md（只增不改）
         # 落点/上限/查重的唯一事实源：agent_ws 的 memory_file/tools_file/topics_dir/changelog_file
         #   + MEMORY_CORE_MAX_* 常量 + append_memory()。mcp_server 只组装视图与分段，不另拼路径。
  技能/  说明.md、论文初稿（少年司法版）.md、综述（少年司法版）.md
         论文初稿（通用暂用版）.md、综述.md、维护综述库.md、跨学科发散与补文献.md
  参考手册/论文初稿（少年司法版）—三大刊结构.md（读取主工作流时自动附带）
  参考格式/ 说明.md
  交付模板/ 交付说明书模板.md
  定时任务/ 说明.md
```
...
完整读取记忆的 **core 视图**（`read_project_memory` 默认 `scope="core"` = 项目记忆.md + 工具经验.md 全文 + 主题档案清单；
**主题正文默认不读**，要用才 `scope="topic:<主题名>"` 单独取——用户要求「旧结论作废、从头重来」时不取即读不到），
再按任务类型、研究领域路由，命中后先读工作流再做。闸门只认 core / all 视图：单独读一份主题档案**不**放行其他工具。两条综述与通用初稿三条工作流在开工前问一次是否派子代理（用户决定，回答前不派）。
少年司法论文初稿走 v6（2026-08-20 上线）：先读本题核心文献的完整原文（检索片段、摘要与 SAC 摘要都不算读过），再照 12 篇三大刊的实测结构搭骨架；
骨架搭好把逐字标题树连同「每章准备写什么」贴给用户看一眼，没有异议就接着写全文，不必停下来等；批量精读、引证核验与定稿前独立校核默认派子代理，起草正文各章不派（跨章约束）。
正文只划禁区、不设配额（9 条禁区＋五条不能破），最终成稿必须交付 DOCX，Markdown 只作中间稿或核验记录。选型决策树、七类原型与准入自检、选型前侦查、两张骨架卡与 18 项可数硬失败随 v6 整体废止。
`read_workflow` 自动附带的参考材料是 `技能/参考手册/论文初稿（少年司法版）—三大刊结构.md`（12 篇逐字标题树与各章实测字数），读的是实际磁盘版本；材料位于子目录、
常量不以 `_WF_` 命名，因此不进入六条顶层工作流列表；主工作流的路线、决策点和可靠性要求优先。旧《成文技艺手册》不再内嵌或生成；升级后的首次脚手架初始化会一次性删除资料库中由应用管理的全部现存副本，包括用户修改件、待合并件和历史备份，完成后写状态避免反复删除用户日后主动创建的同名文件。通用初稿会明确提示
...
- **TOOLS 数量以 `len(TOOLS)` 为准**。分派在 `do_tool()`，绝大多数是对 server HTTP 端点的薄封装；除原有检索、索引、Wiki、研究和记忆工具外，还包括：
  `list_workflows / read_workflow`（强制工作流入口；读取少年司法初稿时自动附带《三大刊结构》台账）、`maintenance_audit`（统一全量体检）、
...
五个顶层 tab（对应 `<main id="panel-*">`）：

| tab | panel | app.js 分区 | 说明 |
|---|---|---|---|
| 📊 库总览 | `panel-dashboard`（`:80`） | `app.js:1022` | 仪表盘，全部手绘 SVG/CSS |
| 📚 文献 | `panel-browse` | `app.js` 的检索与浏览分区 | 浏览、题录查找、全文检索、左侧分类、客观标签/文献性质筛选和选择性深索 |
| 🤖 Agent | `panel-agent`（`:296`） | `app.js:2414` | MCP 接入引导（本机真实命令 + 工具表 + 交付物/资料库入口 + 定时任务） |
| 📖 综述库 | `panel-wiki` | `app.js` 的综合层主题书架 | 按来源自动归类、自定义主题、搜索筛选、打开/核验/删除与页内目录；综述写作和更新交给 Agent |
| 💬 对话 | `panel-chat`（`:505`） | `app.js:2845` | SSE 流式 RAG 对话 |

其余分区：设置弹窗、手动更新、**首启向导**，以及文件夹模式拖入/选择 PDF、EPUB、DOCX、Markdown、TXT 入库。
...
| `server.py` | FastAPI 全部业务端点 + 构建编排 + 深索队列 + 自动更新循环（路由持续演进，不在散文里写死数量） |
| `launcher.py` | pywebview 原生窗口 + server 子进程生命周期 + 原生目录选择器桥；从 PNG 真源生成多尺寸 ICO 并同步窗口/任务栏/安装目录图标 |
| `mcp_server.py` | stdio MCP server；工具、Resources、templates 与 Prompts 以代码和自动生成的 `MCP接入说明.md` 为准 |
...
| `textloc.py` | 引文定位：一段引文 → 哪篇及原文位置；PDF 另带印刷页码 |
...
| `document_formats.py` | 支持格式、扩展名、主全文优先级与兼容字段的唯一事实源；HTML 明确排除 |
| `folder_source.py` | 文件夹模式数据源：扫五类全文文件 + 读 meta_cache（不调 LLM）。**排除 `0_Agent*` 前缀目录** |
| `folder_meta.py` | 文件夹模式：LLM 从全文开头 1-2 个定位单元抽题录（严格 JSON + 兜底） |
| `folder_ingest.py` | 文件夹模式 build 步骤：并发抽题录写 meta_cache（必须先于 index_light） |
| `models_bootstrap.py` | 首启从云端下载两个 INT8 ONNX 模型（归档合计约 900MB，解压后约 1.2GB），校验 sha256 + 解压 |
...
| `extract.py` | F 档 ①：有全文附件的篇 → 通用结构化文本提取；PDF 另做空页本地 OCR → `data/extracted/`（ThreadPool，断点续跑） |
| `deep_extract_status.py` | 全文提取状态 sidecar、旧 `deep_no_text` 迁移与分类计数 |
...
- `server.py` 路由较多，本轮只清点了签名与关键实现；具体数量以代码为准。
- MCP 工具数量不得在散文或注释中复制；统一读取 `mcp_server.TOOLS`，文档由 `gen_mcp_doc.py` 生成并由 `check_guides.py` 校验。
