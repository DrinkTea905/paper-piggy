...
(function () {
  ...
  function agentGuideCard() {
    ...
    return `<div class="howto">
      ...
      <div class="flow">
        ...
        ${arrow}
        ${step("s3", "3", "🔎", "布置研究任务", "先按<b>初稿 / 综述</b>，再按<b>少年司法 / 其他领域</b>路由。少年司法初稿会先完整精读核心文献，再照三大刊实测结构搭骨架；逐字标题树和每章写作计划给你看一眼，没有异议就继续全文。批量精读、引证核验和定稿前校核默认派子代理，正文各章不派。")}
        ${arrow}
        ${step("s4", "4", "📚", "积累研究成果", "把成品保存到<b>交付物</b>，把可复用结论写入<b>综述库</b>。Agent 每次开工先读项目记忆，产生新的长期信息后再写回；每个研究主题另存一份档案，默认不掺进别的题目。")}
      </div>
  ...
  const AG_TOOLS = [
    ...
    ["“记住我偏好脚注 / 这个项目定了 XX”", "append_project_memory", "把你的偏好、已定决策记进项目记忆；研究主题的结论另存进该主题自己的档案，换任何助手接入都从这接着干"],
    ["“这个题结论作废了 / 已经写完了”", "set_memory_topic_status", "把某个研究主题的档案标成进行中 / 已完成 / 已作废，作废的旧结论下次不会再被拿来用"],
    ["“看看有哪些综述该更新了”", "pending_wiki_updates", "拉新文献影响了哪些既有综述页，逐页判断标脏 / 重写"],
  ];
  const AG_PROMPTS = [
    "请用「论文初稿（少年司法版）」研究监督考察条件控制；先完整精读本题核心文献，再参照《三大刊结构》台账搭骨架。把逐字标题树连同每章要回答的问题、使用的材料和准备得出的判断给我看一眼，我没有异议就继续写全文。",
    "请用「综述（少年司法版）」梳理社会观护的理论谱系、制度模式、证据基础和未决争议，每个论断带原文页码。",
    "请用「论文初稿（通用暂用版）」研究自动化行政中的说明理由义务；同时提醒我这套流程尚未经过其他部门法训练验证。",
    "请用「综述」工作流整理库内关于认罪认罚从宽与司法信任的研究，区分规范观点与实证证据。",
    "维护我的知识库和综述主题归类；安全事项直接处理，需要研究判断的集中说明。",
    "帮我把这个题跨学科发散一下，列出需要补充的理论与文献。",
  ];
