...
"""
import sys, hashlib, re, json, difflib, time, os
...
目标：检查 PaperPiggy 使用的 CSSCI、三大刊、北大核心、AMI、SSCI/JCR、TSSCI 与项目偏好目录是否已有新版。

执行步骤：
1. 打开应用「库总览」记录的目录来源链接、当前版本、上次检查日期与下次检查日期。
2. 优先查官方目录；SSCI/JCR 同时核对 ShowJCR，中文目录可参考 cnki-Scholar，但第三方数据只能作交叉核对。
3. 将新增、删除、改名和分区变化逐条列出；不要直接改目录文件，也不要用模型猜测缺失条目。
4. 把核对报告写入 `0_Agent交付物/定时任务/文献目录半年检查/目录检查_<日期>.md`。
5. 如确需更新目录，交由开发者审阅来源和许可证后，在开发仓库中更新、测试并发布新版。

说明：PaperPiggy 只登记和展示这个任务，**不会自行联网执行**。需要用户在所用 AI 助手的日程系统中启用；未接入调度器时，“启用”不代表它正在自动运行。
"""
...
2. 调 `maintenance_audit`，一次盘点模板升级、索引、深索/PDF/OCR、检索摘要、wiki 更新建议和 wiki 体检。
...
2. **索引与深索**：简单事项直接处理，安全重试和增量任务直接继续；清空重建索引、删除数据等破坏性动作必须先问。缺附件、坏 PDF、OCR 终态列为外部阻塞，不冒充已修复。
...
按 DOI 或规范化题名去重，记录每条候选来自哪篇核心文献以及 PDF 页码或参考文献序号。先用库内检索确认候选是否已经入库，
...
_FACTORY_HASHES = {
    ...
    "rely/定时任务/文献目录半年检查/任务.md":
                                            {"b6aa43603681dc801c3870e15cdb6b58f13444ae"},   # v1 半年目录审查
    ...
    "rely/技能/写论文与综述.md":        {"5027f5d8e6c6837907e5ddbc294de7b2f10d5de3",
                                             ...
                                             "6f1c82b020fad509e00b8bf956f88d34d5e81429"},   # v4 参考链+使用缺口
    "rely/技能/维护综述库.md":          {"ada886d3bf34277de35f6530a02874efcffaac92",
                                             "25646a2433e3a913d32803e3bf895f062424bba2",
                                             "8fd036b38a135c0a7e4e930e3a6b3a3cc74c22b8"},   # v2 全量维护闭环
    "rely/技能/跨学科发散与补文献.md":  {"5dc99c7cd7cac23354e6915eee406e6b47e42a8e",
                                             ...
                                             "9d11e6c36bbf7fb7d87a780648024f77834eda2e"},   # v4 参考链+偏好+缺口
...
def _template_specs():
    ...
    return [
        ...
        ("rely/定时任务/文献目录半年检查/任务.md",
                                             tasks_dir() / "文献目录半年检查" / "任务.md",
                                                                                   _CATALOG_CHECK_TASK,   None, False),
