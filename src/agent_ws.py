...
### 四条研究工作流
- `论文初稿（少年司法版）.md` —— 少年司法论文初稿；默认先给教义学、理论—制度建构两张路线方案卡，用户选路前不写全文；最终成稿必须交付 DOCX，Markdown 不能替代。
...
> 最终论文成稿必须交付可正常打开的 `.docx`；Markdown 只可保存中间稿、写作设计和核验记录，不得作为最终成稿。
...
12. **DOCX 交付与沉淀**：最终论文成稿固定写为 `0_Agent交付物/<主题>/<题目>.docx` 并附 README；`.md` 只可作为中间稿、写作设计、章节功能表、分节论证契约、来源核验或五轮修改记录，不能替代最终成稿。优先采用 `0_Agent资料库/参考格式/` 中的 DOCX 范本；修改既有 Word 文档时保护 Zotero 引注域，不重建文档。交付前实际打开或渲染检查文件可读、标题层级、脚注/尾注、分页和基本版式；当前环境无法可靠生成或验证 DOCX 时，必须报告阻塞，不能把 Markdown 标成完成。按既有主题更新/新建 wiki 页，调用 `set_wiki_theme` 设置主题并维护互链，处理本轮新读来源触发的 wiki 待办。
...
- 没有任何硬失败；最终 `.docx` 已落盘并经打开或渲染检查，Markdown 未被当作最终成稿；用户文件无裸 key；待核项、文献使用与缺口、wiki 主题归类与维护待办已处理。

## 最终报告
- 报告所选路线及跳过/选择依据、两个人工闸门记录、最终 DOCX 路径及打开/渲染检查结果、写作设计和实际创新、结构原型与章节功能、使用来源和页码核验、少年司法闸门、五轮修改、反方与边界、待核项、wiki 写回和仍需用户决定的事项。
"""
...
> 最终论文成稿只以 `.docx` 交付；Markdown 可作中间稿和审计记录，但不算最终成稿。
...
- [ ] 最终论文文件是可正常打开的 `.docx`；Markdown 只保存中间稿或核验记录，没有被当作最终成稿。
- [ ] 已优先采用 `0_Agent资料库/参考格式/` 中适用的 DOCX 范本；修改既有 Word 文档时保护 Zotero 引注域，没有重建文档。
- [ ] 已实际打开或渲染检查标题层级、脚注/尾注、分页、中文与西文字体及基本版式；发现异常后已修正并复查。
- [ ] README 和最终报告明确列出 DOCX 路径与检查结果；无法可靠生成或验证 DOCX 时没有虚报完成。
...
_FACTORY_HASHES = {
    ...
    "rely/技能/说明.md":                {"de19ded63470c2c7975a19eb0f012f14214b62a7",
                                             ...
                                             "399d9c21475aba81a83b1ef0b9257ff686bb0999"},   # v5 DOCX 成稿说明
