...
用户说「**写蓝图**」「**施工图**」「**施工蓝图**」「**先出蓝图**」时，题材同上，但**交付形态换成施工蓝图**——不写全文，见第六节。
...
**触发**：用户说「写蓝图」「施工图」「施工蓝图」「先出蓝图」。

**这时不写全文。** 交一份施工蓝图：大纲、每一节要写什么、每一处引证的完整出处，全都列齐——**只差用语言把它组织出来**。

### 为什么这一节的要求跟别处不一样

蓝图是**要发给另一个 AI 去落笔的**，而那个 AI **没有这个知识库、调不了任何工具、也读不到本工作流**。

于是只剩一条铁律：**凡它要用到的东西，蓝图里都得有；凡蓝图里没有的，它就只能编。**下面每一条要求都是从这里推出来的，不是格式洁癖。

尤其是：**绝不写裸 key**。`H3HVZ5YW` 这种标识只在本知识库内有意义，下游拿到就是一串乱码，而它多半不会说「我查不到」，只会顺着编一个像样的出处出来。

### 蓝图的固定开头（逐字照写，别改）

蓝图开头必须原样带上下面这段——它同时是给下游 AI 的任务指令和格式约束：

````markdown
# 施工蓝图：<论文题目>

> **给接手的 AI：请根据本施工蓝图完成论文初稿，以 DOCX 格式交付。**
>
> 本蓝图已把全文大纲、每一节要写什么、以及每一处引证的完整出处都列好。
> 你要做的是**用学术语言把它组织成正文**，不是重新研究。
> **不要改变蓝图给定的判断，不要新增蓝图里没有的引证**——蓝图里没写的文献你无从核对，
> 写进去就是编造。确有必要补充时，单独列出来说明，不要混进正文。

## 〇、格式要求（先读，全文适用）

**数字**：一律用阿拉伯数字（人数、件数、篇数、条文数量、比例、年月日、印刷页码，如「34 人」「2026 年 7 月 31 日」「第 133 页」「90% 以上」）。**五类例外保持中文数字**：
1. 法条条号与款项序号——第二百八十七条之一、第十七条第三款；
2. 章节标题序号——一、二、三与（一）（二）（三）；
3. 编号词——一是、二是、三是；
4. 固定语与专名——一事不再罚、两高、三大法系；
5. 引语与法条文献题名的原文照录。

判断口径：**外部可核验的计量用阿拉伯数字**（法条项数、次数、人数、页数、篇数）；**本文自身论证结构的列举用中文数字**（三项成因、两条路径）。

**引注**：同一文献第二次、第三次出现时，**照写完整题录**（作者、题名、出处、年份、页码），页码随实际引用处变。**禁用**「前引」「参见前引文」「同前注」「同上注」「Ibid.」——一处都不许有。

**称引**：正文提及他人研究**不写姓名**，改用「有观点认为」「有论述指出」「另一种意见主张」，或直接引原话加引号；作者姓名与职称只出现在脚注题录里。三项例外：① 借用他人自造并加引号的学术命名时**必须具名并加注**；② 自引写「笔者曾主张」；③ 内部材料仍具名到人。

**引号**：正文一律全角 “”（内层 ‘’），不用「」，也不用半角 " 与 '。**没有例外**——我国台湾地区的法规名同样用全角引号。交稿前可直查码点：正文里 U+0022 与 U+0027 应为 0。

**其他禁区**：正文不用破折号；不用「或许」「似乎」「在一定程度上」这类对冲词；不用「值得注意的是」「尤其重要的是」这类划重点词；不写作业过程（不写「本文将先……再……」式的路线图交代）。

**交付**：最终成品交 **DOCX**，不是 Markdown。
````
...
**一、全文骨架**——逐字标题树，标到实际要用的层级（多数文章两级即可，23/28 篇不用三级）。每个一级标题后标注该章的预计字数区间。

**二、逐节施工单**——**这是蓝图的主体**。每一节一块，逐节写：

````markdown
### 1.2 <这一节的逐字标题>

- **本节回答**：（一句话，说清这节要解决哪个问题）
- **用什么材料**：（列出本节要用到的文献、法条、数据，各带完整出处）
- **得出什么判断**：（一句话，说清读者读完这节应当接受什么）
- **展开顺序**：（三到五个要点的先后，说明为什么是这个顺序）
- **引证清单**：
  - [1] 宋英辉、刘铃悦：《低龄未成年人犯罪核准追诉问题研究》，《法治研究》2022 年第 3 期，第 68 页。
    → 用来支撑：「核准追诉的实质是由最高人民检察院决定案件走向」
  - [2] 《中华人民共和国刑事诉讼法》（2018 年修正）第二百八十三条。
    → 用来支撑：「附条件不起诉的考验期为六个月以上一年以下」
- **注意**：（这一节特有的坑，比如某个说法有争议、某个数据口径要交代）
````

**三、全文引证总表**——把全部引证汇成一张表，每条含：序号、完整题录、印刷页码、出现在哪一节。下游 AI 靠它统一体例、检查有无遗漏。

**四、不确定与待核项**——把**没核实的、有争议的、你拿不准的**单独列在这里，写明不确定在哪。**绝不能混进施工单让下游误以为已经定了。**

### 硬要求

1. **每一处引证都写到印刷页码。** 蓝图里的页码是下游唯一的依据，它没法回原文查。推算所得的页码要标「（页码推算）」。
2. **每一条引证都写明「用来支撑哪句判断」。** 只给一串文献而不说各自干什么，下游必然张冠李戴。
3. **判断必须是你已经做出的判断**，不是留给下游去想的问题。蓝图里出现「此处可探讨……」等于把活推回去了——那正是这份蓝图要消灭的东西。
4. **前面各节的约束照旧适用**：五条不能破、第三节 9 条禁区、§5 少年司法 12 条。其中下游用得着的部分（数字、引注、称引、引号、禁区）已摘进上面的固定开头；**不要指望下游能读到本工作流**。
5. **蓝图本身交 `.md`**（下游 AI 好读、好复制），落在交付物目录；**并在开头写明成品要交 DOCX**。
6. 蓝图仍然要走《检索发现日志》——**读文献这一步一步都不能省**。蓝图只是省掉了「用语言组织成文」，不是省掉研究。

### 交蓝图时的完成标准

- 骨架逐字标题树完整，每一节都有施工单，没有空着的节。
- 引证清单里**零裸 key**，每条都有完整题录与印刷页码，每条都注明支撑哪句判断。
- 不确定项单独成节，没有混进施工单。
- 固定开头一字不缺（下游 AI 的格式约束全靠它）。
- 交付物里同时说明：这是蓝图不是成稿，成稿由下游 AI 按蓝图完成并交 DOCX。
...
_FACTORY_HASHES = {
    ...
    "rely/技能/论文初稿（少年司法版）.md": {"4a375d4525583e618d4785c718e720e0881afbc5",   # 2026-08-21 引号规则去掉台湾地区例外 + 示例改真全角
                                            "645ca9fa2dd53dd4e9d15bdfeeb7611ccf2ef9ae",   # 2026-08-21 v6 新增「施工蓝图」交付形态
                                            "1ec4ac1b05d298a9293151cf296e6aa93227e6d2",   # 2026-08-20 v6 工作流上线
...
_WORKFLOW_FACTORY_EXACT_HASHES = {
    "rely/技能/论文初稿（少年司法版）.md": {
        "1c5e3856bdcf6929c022d88b63b5ae0eef397568",   # 2026-08-21 引号规则去掉台湾地区例外 + 示例改真全角
        "c56b37b96aacefa194f661bfd6696501fc00d929",   # 2026-08-21 v6 新增「施工蓝图」交付形态
