...
### 四条研究工作流
- `论文初稿（少年司法版）.md` —— 少年司法论文初稿；先走选型决策树，默认先给两张骨架卡（套不进七类原型时自建骨架，与原型同等），用户选定前不写全文；最终成稿必须交付 DOCX，Markdown 不能替代。
...
- 少年司法论文初稿先走选型决策树并对候选类型做准入自检，默认先提交两张骨架卡；本题套不进七类结构原型时自建骨架，与选用原型同等，不得硬套。明确推荐一条并说明理由，用户选择前不得起草全文。
- 四条研究工作流均在开工前问一次是否派子代理、派在哪些环节，由用户决定；用户回答前不派，用户说“你自己做”就全程不派，标准一条不减。
...
> `read_workflow` 自动附带《成文技艺手册》：手册提供选型决策树、8 张真实骨架卡＋第九张自建卡、句型库、标题词组池、少年司法 12 条与不得挪用的命名表。本文件的两个人工闸门、证据要求与硬失败优先于手册。
...
本工作流不预设你一个人做完。**开工前把下面五处一次性列给用户，问哪些环节派子代理、哪些你自己做；用户回答前不要派。** 用户说"你自己做"就全程不派，标准一条不减；**用户没答复也不派**——默认建议是给他选的参考，不是他沉默时的授权。最终报告里写明实际分工。

| 环节 | 默认建议 | 理由 |
|---|---|---|
| 选型前侦查：核心文献摸底 | **建议派** | 一人 4—6 篇，只回交题录、逐字标题树与选型七问中它答得了的那几条；这一步读的是局部，不产出可引用论断，合并判断由你做 |
| 证据底座：核心文献逐篇完整 `read_source` | **建议派** | 收益最大：逐篇读到终页最占上下文，各篇之间互不依赖。每个子代理一次 4—6 篇，回交文章卡（题录／印刷页区间／可引论断＋页码／材料身份标注），不得只读题录、摘要、SAC 或检索片段 |
| 第一闸门：两张骨架卡 | **建议派**（一卡一个，互不通气） | 同一上下文写两张卡，第二张几乎必然是第一张的变体——这正是"两条路线其实是一条"的来源。隔离上下文才拿得到真差异；收卡后由你逐张核准入自检与第 4 条的差异要求 |
...
**没读文献就出骨架卡，是拿题目猜结构。** 但这一步只为一件事服务：把选型决策树与准入自检问到的那几条，从"印象"变成"有据可查"。它**不是**综述，也不是证据底座。

**读多少（硬上限，超了就是耽误事）**
1. 先建候选池：题录级 20—40 条，只看题名、期刊、年份、摘要，用来认地形——**这不算"读"**。
2. 从池子里挑 **6—10 篇**核心，其中：
   - **完整逐页读 2—3 篇**：挑"材料最新的"和"最被引或最对立的"。这几篇决定最长章在哪、对手是谁；
   - 其余只读**局部**：逐字标题树（各级标题原文）＋引言＋结论。**看同行怎么分章，比看他说了什么更有用。**
3. 现行法与关键司法解释：条文要核到现行版本与法制时点，不计入上面的篇数。
4. **读到 10 篇仍定不下选型，就停下来报告**"这题的现场比预想复杂"，把卡住的那一问交给用户，不要靠继续读拖下去。

**读来回答什么（只回答这七问，其余先记着别展开）**
① 这个对象在中文法学核心期刊标题里有没有通行名称？（决定类① 上户口型能不能用）
② 现场有几派、争的是不是同一件事？**具名到人**。（决定类③ 定性辨异型）
③ 有没有成批条文／司法解释／地方试点可引？（类①、类⑤ 的材料门槛）
④ 有没有一个明确的对手学说、能整段引它的原话？（决定类⑦ 换轨型）
⑤ 列不列得出 ≥4 处空缺，且本文不打算做完任何一项？（决定类⑤ 清点型）
⑥ **哪一块材料最新、最密、你最有话说？**——它就是最长章，位置应落在第 2—4 单元。
⑦ 现有文献里有没有人已经把你要写的这篇写过了？（有就先改题或改角度，别拖到起草才发现）

**产出：一页《选型前侦查表》，与两张骨架卡一起交给用户**
- 候选池条数、完整精读的篇名、局部阅读的篇数；
- 上面七问逐条作答，**每条挂上是哪篇文献／哪条法源支撑（带印刷页）**；无法回答的写"未检出"，不写"学界没有"；
- 一句"本题现在最缺的材料是什么"。

**边界（写死，别越）**
- **侦查阶段的局部阅读不产生可引用论断。** 正文里每一个实体判断仍必须走「证据底座」的完整精读并核到印刷页；侦查读到的东西只能用来**选结构**，不能直接落成引注。
- 侦查阶段不写综述、不建材料三表、不做归属分层——那些是证据底座的活。
- 侦查表里的每一条都要能追到具体文献；准入自检的出局理由**必须引用侦查表的条目**，不得凭印象判某型出局。

## 第一闸门：选型 → 两张骨架卡（用户选定前不得写摘要、引言或任何章节正文）

> **两卡来源有三种组合，地位完全平等：原型＋原型、原型＋自建、自建＋自建。**
> 七类原型归纳自陈瑞华 36 篇刑事诉讼法论文（2001—2026）——一个作者、一个部门法；手册 §2 自己就记着一篇（`LD68HVXY`）归不进任何一张卡。**一个作者的 36 篇尚且装不下他自己，本题装不进去毫不意外：套不进去时自建骨架是正解，硬套才是错的。** 被放宽的只是"卡从哪来"；底盘八条、第二闸门、证据底座、硬失败与完成标准一条不减。

1. **准入自检（先做这一步，再决定卡从哪来）。** 走手册 §1 决策树 Q1→Q5 得到候选类型后，对每个候选逐条核准入；命中任一条即**该型出局**，且出局理由必须写成一句具体的话（写"基本适配""大致可用""略作调整即可"视为本步未做）：
   - 走到该叶子所依赖的那条判据其实不成立（类① 要有成批条文／司法解释／地方试点可引；类② 要末章真写得出"不选边"的最后一句；类③ 要现场至少两派在争；类④ 要预先写得出三条别人打你的话；类⑤ 要列得出 ≥4 处空缺且本文不打算做完任何一项；类⑦ 要有明确的对手学说）；
   - 手册该卡的「**不可调换**」与本题材料的必然顺序冲突（例：本题的效力争议必须先讲，定性章无法前置）；
   - 命中该卡的「**不适用警告**」（已认定唯一答案却想用类②、没有对手学说却想用类⑦）；
   - 你写不出该型第 ⑨ 栏的三项打死条件，或写出来发现本题已经缺了其中一样。
   **七类全部出局、或只剩一类，都是正常结果，不是失败**——直接按第 2 条自建。不得为了凑满两张卡而放行一个已判出局的型。
2. **自建骨架卡（与原型卡同等，不是兜底）。** 按手册 §2 卡⑧「自建骨架」立骨，栏目与原型卡同格式逐项落成文字成品，并另加两栏：
   ⑩ 像哪两型、具体在哪一点上不像（写得出差异，不写"综合了几型的优点"）；
   ⑪ 本卡偏离了底盘八条的哪几条、依据本题的哪份材料（不偏离就写"无"）。
   底盘八条同样可以偏离，但**必须在 ⑪ 里写明偏离哪条、凭什么材料**；静默偏离按硬失败处理。
3. 每张卡逐项落成**文字成品**（原型卡照手册对应卡，自建卡照第 2 条），出现"待定""视情况"即视为该栏未交：
   ① 完整拟题（主标题＋副标题；副标题按手册 §3 公开限定射程，例："——兼论……的事实认定问题"）；
   ② **逐字一级标题树**，含序号与标点，逐章标注节数，写明哪些章不分节；
   ③ **每章、每节准备写什么**——这一栏是骨架卡与提纲的分界：提纲只说"讲什么"，这里要说清"怎么讲"。
      每个一级单元 **2—3 句**，有节的每节再补一句，逐条交代三件事：
      **本章回答哪个问题｜用哪类材料回答（规范／理论／事实，并注明这材料现在手上有没有、出处在哪）｜准备得出什么判断**（判断尚未定的，写出方向与备选，不写"待定"）；
      判据：**遮住标题后，这段话仍能认出是哪一章**——写成"介绍……的基本情况""对……进行分析""梳理相关争议"一律视为该栏未交；
      有节的还须写出每节**区别于同章其他节的那一点**；写不出就是这一节没有独立存在的必要，合并掉。
   ...
   ⑧ 本卡的**不可调换处**（原型卡照抄该栏并改写成本题的话，例："定性章必须在辨异章之前"；自建卡自己写出，并说明凭本题哪份材料）；
   ⑨ 会打死本卡的三项材料条件（缺哪三样材料这张骨架就不成立）。
4. 两卡差异必须落在①②③④⑦：**一级标题同名不得超过 1 个；章数不得相同**——两卡章数确实都被本题材料锁死在同一数字时，须写明锁死它的是哪份材料，并做到①②③⑦四栏全不同。只换理论名或调换章序不算两条路线；**③ 两卡各章准备写的内容雷同，等于只有一条路线**。
5. 推荐一条，理由只讲三件事：材料够不够撑起最长章、末章能否写成该卡要求的形态、缺口在哪。
6. 用户已明确选定结构（原型或自建皆可）且明确表示不需要比较时，才可跳过双卡；缺任一条件不得跳过。

## 证据底座（选型后的完整精读）

> 与「选型前侦查」的分工：侦查读局部、只为选结构，**读过的东西不能直接落成引注**；本节才是可引用材料的唯一来源。侦查里只读了标题树＋引言结论的篇目，进正文引用前必须在这里补完整精读。骨架卡第 ③ 栏说"手上没有"的那些材料，在这里补齐或据实降级。
...
**每一次 `search_localkb` 都要留一行。** 应用本身不记录检索历史（`/search` 与 `/chat` 都不落盘），这份日志是"什么查询该返回什么文献"的**唯一真实记录**——不记，这些判断就永久丢失，知识库的检索质量也就永远没法校准。

落盘 `0_Agent交付物/<主题>/检索发现日志.md`，与成品同目录。**表头逐字照抄，不改列名与顺序**：

| 轮次 | 查询词（逐字） | 名次 | key | 题名 | 处置 | 理由 |
|---|---|---|---|---|---|---|

- **查询词逐字照抄**你实际发出去的那一串，不要事后润色；同一问题换了检索式就另起一轮，轮次递增。
- **名次**＝这条结果在本次返回里的位置（1、2、3…）；返回不足则记实际条数。
- **处置**只用三个词：`采用`／`弃用`／`待核`。
- **理由**一句话。**弃用理由比采用理由值钱**——它是负例，写清到底为什么不要（"同题不同版本""只是背景不进正文""程序阶段对不上""读了全文发现不相关"），不要只写"不相关"。
- 另起一节记**漏召回**：靠脚注追踪、他人推荐或人工翻找才补上、而检索从没返回过的文献，逐条记题名、key，以及"你原本用的哪条查询词本该找到它"。**这类数据最稀缺。**

选型前侦查与证据底座两个阶段都要记；派了子代理的，子代理各自交回自己那份，由你合并。**不得事后凭记忆补写整份日志**——当轮没记就写"本轮未记录"，不要伪造名次。

## 交付与沉淀
1. 最终论文成稿固定写为 `0_Agent交付物/<主题>/<题目>.docx` 并附 README；同目录附《检索发现日志》；`.md` 只可作中间稿、骨架卡、落位表、来源核验或修改记录，不能替代最终成稿。
...
2. 未过第一闸门即写正文；两卡同名标题 ≥2；两卡章数相同而未写明锁死它的材料、或写明后①②③⑦仍有一栏相同。
2-b. 准入自检判某型出局却仍拿它交卡；或自建卡偏离底盘八条而未在 ⑪ 栏写明偏离哪条、凭哪份材料。
2-c. 未做选型前侦查即提交骨架卡；或侦查七问有任一条没有文献／法源支撑（含"未检出"也要写明检索式）；或骨架卡第 ③ 栏出现"介绍……的基本情况""对……进行分析"式空话。
...
## 用户决策点
- 分工：开工前问一次哪些环节派子代理；用户回答前不派，用户说"你自己做"就全程不派。
- 第一闸门：《选型前侦查表》与两张骨架卡**一起**交给用户，由用户选定一张（原型或自建同等）；未选不得起草摘要、引言或任何章节正文。侦查读到 10 篇仍定不下选型时，把卡住的那一问交给用户，不要继续读。
...
## 最终报告
逐项列上述 18 个数字与判定、选型前侦查的规模（候选池条数／完整精读篇名／局部阅读篇数）与七问作答、准入自检结果（哪几型出局、各凭哪一条、引侦查表第几条）、两张卡的来源（原型／自建）与所选及被否理由、骨架卡第 ③ 栏在成稿里的兑现情况（哪几章写的与当初说的不一样、为什么）、两个闸门记录、实际分工（哪些环节派了子代理、各几个、独立校核由谁做）、《检索发现日志》的路径与行数（含漏召回几条）、DOCX 路径与打开检查结果、少年司法 12 条各改变了哪一句、偏离基准处及依据、待核项与 wiki 写回。
"""
