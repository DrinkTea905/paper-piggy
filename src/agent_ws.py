...
- 旧版《…—成文技艺手册.md》已彻底停用，不再随应用分发。升级到当前版本时，应用会删除资料库中
  由它管理的全部现存副本，包括用户修改件、待合并件和历史备份。
- 仍在使用的参考材料和工作流可以由你定制。升级时用户修改会原样保留，新版集中放进 `升级与备份/待合并/`。
...
# 已停用手册不再保存正文或加入出厂模板；以下名称只用于升级时执行用户明确选择的彻底清理。
_OBSOLETE_JJ_DRAFT_HANDBOOK_FILENAME = "论文初稿（少年司法版）—成文技艺手册.md"
_OBSOLETE_JJ_DRAFT_HANDBOOK_STATE = "obsolete-jj-draft-handbook-removed-v1"
...
_FACTORY_HASHES = {
    ...
    "rely/技能/说明.md":                {"1a5f6069767bb8e7e1939509d6027c49b69319e7",   # 2026-08-20 v6 工作流上线
                                            "de19ded63470c2c7975a19eb0f012f14214b62a7",
                                             ...
                                             "a662d0579c349748c190cf0cbb1c52512b174813",
    },   # 历史版本均保留；当前版明确彻底移除停用手册
...
def _is_obsolete_jj_draft_handbook_copy(name):
    """只匹配应用曾管理过的手册主件、旧旁本、待合并件和历史备份。"""
    p = Path(name)
    if p.suffix.lower() != ".md":
        return False
    stem = p.stem
    base = Path(_OBSOLETE_JJ_DRAFT_HANDBOOK_FILENAME).stem
    if stem == base or stem.startswith(base + ".user-backup-"):
        return True
    for suffix in (".new", ".新版待合并"):
        if stem == base + suffix:
            return True
        prefix = base + suffix + "."
...
def _remove_obsolete_jj_draft_handbook():
    """一次性删除停用手册的全部应用管理副本，包括用户修改件。

    这是用户明确选择的破坏性迁移。清理范围严格限制在 ``0_Agent资料库``；成功后写入状态，
    避免用户日后主动新建同名文件时仍被每次启动反复删除。任一删除失败都不写完成标记，下次重试。
    """
    state = _load_update_state()
    if state.get(_OBSOLETE_JJ_DRAFT_HANDBOOK_STATE):
        return {"status": "already_done", "removed": 0, "failed": []}
    ...
    try:
        for dirpath, dirnames, filenames in os.walk(root, topdown=True, followlinks=False):
            ...
            for filename in filenames:
                if not _is_obsolete_jj_draft_handbook_copy(filename):
                    continue
    ...
    if not failed:
        state[_OBSOLETE_JJ_DRAFT_HANDBOOK_STATE] = {
        ...
        try:
            ...
            failed.append("<状态写入失败>")

    status = "partial" if removed and failed else ("removed" if removed else ("failed" if failed else "absent"))
    if removed:
        print(f"[agent_ws] 已按用户选择删除停用的成文技艺手册及其副本：{len(removed)} 份",
              file=sys.stderr, flush=True)
    return {"status": status, "removed": len(removed), "failed": failed}
...
def ensure_scaffold():
    ...
    try:
        ...
        acts["migration/移除停用成文技艺手册"] = _remove_obsolete_jj_draft_handbook()
