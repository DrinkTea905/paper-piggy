...
组装可分发 bundle（开发机跑）。产出 dist/LocalKB/：
  ...
  run_localkb.py / 启动.bat / PaperPiggy.vbs  启动器（设 env→首启下模型→开原生窗口）
  ⚠️ run_localkb.py 的**文件名不能改**：config.py 靠它判定「是不是分发包」（见 config.py 的 is_bundle）。
...
def write_launchers():
    ...
    (BUNDLE / "PaperPiggy.vbs").write_text(VBS, encoding="gbk")
    print("[bundle] 启动器就绪：run_localkb.py / 启动.bat / PaperPiggy.vbs")
