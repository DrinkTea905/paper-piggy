...
| 文件 | 做什么 |
|---|---|
| `论文初稿（少年司法版）.md` | 先完整精读本题核心文献（检索片段、摘要与 SAC 摘要都不算读过），再照 12 篇三大刊的实测结构搭骨架；骨架完成后先提交逐字标题树，并逐章说明回答什么问题、使用什么材料、得出什么判断，用户没有异议就继续起草全文；读取时自动附带《三大刊结构》台账，批量精读、引证核验与定稿前独立校核默认派子代理，正文各章不派；最终成稿交付经检查的 DOCX |
