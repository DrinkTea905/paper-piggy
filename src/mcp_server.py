...
- 研究任务先按任务类型（论文初稿 / 综述）、再按领域（少年司法 / 其他）路由到四条研究工作流。少年司法论文初稿先做选型前侦查（挑 6—10 篇核心：完整精读 2—3 篇、其余局部读；局部阅读不得落成引注），再走选型决策树并对候选类型做准入自检，默认先给两张骨架卡（含每章每节准备写什么）；本题套不进七类结构原型时自建骨架，与选用原型同等，不得硬套。明确推荐一条并说明理由，用户选择前不得起草全文。只有用户已明确结构且明确不要比较时才可跳过双卡。
- 四条研究工作流都在开工前问一次是否派子代理、派在哪些环节，由用户决定；用户回答前不派，用户说“你自己做”就全程不派，标准一条不减。
