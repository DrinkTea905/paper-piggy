...
- 研究任务先按任务类型（论文初稿 / 综述）、再按领域（少年司法 / 其他）路由到四条研究工作流。少年司法论文初稿先读本题核心文献的完整原文（逐页正文与印刷页码，检索片段、摘要与 SAC 摘要都不算读过），再照自动附带的《三大刊结构》搭骨架；骨架搭好把逐字标题树连同「每章准备写什么」贴给用户看一眼，没有异议就接着写全文，不必停下来等。
...
TOOLS = [
    ...
    {
        "name": "read_workflow",
        "description": "读取指定工作流全文，并自动附带该工作流登记的参考材料。开始写作、维护或跨学科发散前必须先读匹配工作流并照完成标准执行。",
        "inputSchema": {"type": "object", "properties": {
    ...
    if name == "read_workflow":
        ...
        for _key, path, _text, _mask, _seed in AW.workflow_companion_specs(workflow.name):
            out.extend([
                ...
                "",
                f"自动附带参考材料：{path}",
                "本材料不是独立工作流；主工作流的路线、决策点、证据要求和完成标准优先。",
                "",
            ])
            try:
                out.append(path.read_text(encoding="utf-8"))
            except Exception as e:
                out.append(f"【自动附带失败】应读取该材料但无法打开：{type(e).__name__}: {e}")
        return "\n".join(out)
