...
| **用户的正式安装** | `D:\PaperPiggy\` | 👀 **只读**（看日志、看文件是否存在、复现问题）<br>⛔ **绝不修改任何文件**，不改代码、不改配置、不删数据、不"顺手帮他修一下" |
...
- **Agent 层**：通过 MCP（工具数以 `mcp_server.TOOLS` 和自动生成的 `MCP接入说明.md` 为准）让 Claude Code 等外部 agent 直接操作这个知识库。研究任务先按任务类型、再按领域路由到「论文初稿（少年司法版）」「综述（少年司法版）」「论文初稿（通用暂用版）」「综述」；另有「维护综述库」「跨学科发散与补文献」两条支持工作流

少年司法论文初稿走 **v6**（2026-08-20 上线，取代 v3/v4 血统那版）：先读本题核心文献的完整原文（逐页正文与印刷页码，检索片段、摘要与 SAC 摘要都不算读过），再照 12 篇三大刊的实测结构搭骨架；骨架搭好把逐字标题树连同**每章准备写什么**（回答哪个问题｜用什么材料｜得出什么判断）贴给用户看一眼，没有异议就接着写全文，不必停下来等。**选型决策树、七类结构原型与准入自检、选型前侦查、两张骨架卡、18 项可数硬失败，随 v6 整体废止**——v4 的决定性教训是：可数指标可以被优化到全绿而质量并不改善，前向验证才是唯一验收。正文只划禁区、不设配额（9 条禁区＋五条不能破），其中「把作业过程写进正文」「结论进正文、出处样本与限度进脚注」「制度建议要嫁接不要发明」三条来自 2026-08-20「用户亲手成稿 vs 同题 AI 稿」的对照。`read_workflow` 自动附带的参考材料由《成文技艺手册》换成《三大刊结构》台账，手册停用不删。批量精读文献、引证核验、定稿前独立校核默认派子代理，起草正文各章不派（跨章约束）。通用初稿必须提示“尚未经过其他部门法训练验证”。旧 `写论文与综述.md` 迁移时，历史出厂原文可升级到 `综述.md`，用户改过的文件必须改名保留，绝不删除或覆盖。
...
```powershell
& .\build\py312\python.exe -m unittest discover -s src\tests -t src\tests -p "test_*.py"
```

> `src\tests` 下没有 `__init__.py`，所以 `-t src` 会报 `Start directory is not importable` —— 顶层目录要写 `-t src\tests`。
...
**① 改完文件，提交前先看 `git diff --stat` 的行数对不对。**
本仓库 `core.autocrlf=false`，且多个长文件（`agent_ws.py`、`mcp_server.py`、`index.html`、`docs/*.md`）在 HEAD 里就是 **LF / CRLF 混合**。
很多编辑器（含 AI agent 的编辑工具）会在写回时把整文件行尾统一 —— 于是改一行文案，`git diff` 变成三千多行的全文件重写，**真实改动完全无法 review**。
它不影响任何 hash（Python 读源码走 universal newlines，字符串常量里始终是 `\n`；`check_guides ④b` 与 sha256 都实测无变化），所以不会有任何报错来提醒你 —— 只能靠自己看 stat。
判据：改了 N 处文案，`git diff --stat` 就该是 N 量级；出现百行以上就是行尾被统一了，逐行还原后再提交。（2026-08-10 实测：3756 → 205。）
...
- **期刊权重加成：API 模式已校准，本地与 light 未校准**（2026-08-10 复核订正）
  → **旧表述已作废**，原文是「顶刊只多 0.375 分，形同虚设，等用户攒够 20+ 条真实查询再校准」——**三处都不准**，且它把一件已完成、结论相反的事冻了 34 个版本（1.0.29 → 1.0.53），每次新对话都在提醒用户一件错的事。留在这里当反面教材（§4 的「散文里冻住会变的事实」原地复发）。
  → **当前事实**：`config.WEIGHT_BONUS_SCALE_API = 0.30`（v1.0.29 / 2026-07-19 用 38 条金标集校准所得，方向是**从 0.50 调小**）、`WEIGHT_BONUS_SCALE_LOCAL = 0.50`（未校准）；`retriever._blend_bonus()` 优先用连续 `journal_weight∈[0,1]`，回退旧离散 `TIER_BONUS`。0.375 是**本地口径**；用户实际跑 API（`settings.json` 的 `backend=api`），顶刊绝对加成 +0.30，加在 reranker 的 **0~1** 分上，**不是虚设**。`retriever.py:1029` 的 light 档 `×3` 是无名字面量，**从未校准**。
  → **校准台已有**：`src/tests/retrieval_calibrate.py` 离线重放冻结快照，9 毫秒、零 API、不重建索引。`choose_scale` 只读 calibration 组，validation 只在显式传 `--validate` 时算——**这是防盲测泄漏的设计，别拿盲测组选参**。
  → **实测 A（2026-08-10，旧档位 / 候选快照 v1）**：调参组(n=18) 最优 0.30（nDCG@8 0.9144）；盲测组(n=12) 最优 0.10（0.9176，线上 0.30 为 0.8851）。**两组打架 = 秤不够灵敏**（每条查询只约 2 篇标准答案，top-8 必然装下，recall/must_hit 在 0—0.3 全满分）。
  → **实测 B（2026-08-10 晚，档位修正后 / 候选快照 v2）**：期刊档位改动后 978 个候选里 375 个权重变了，重跑校准——**`choose_scale` 自己选出 0.00**；调参组 0.00 最优（0.9048，0.20/0.30 的 hit@3 从 1.000 掉到 0.889），盲测组 0.10 最优（0.9150，0.30 掉到 0.8571、0.50 掉到 0.8280）。**两组方向首次一致：都指向 0—0.1，线上的 0.30 明显偏高。**
  → 结论方向：**"加成太小要调大"被两轮数据一致否定**；档位越准，越不宜把它掺进相关性排序（档位在「推荐优先」「期刊等级优先」排序与 min_weight 过滤里仍硬生效，不受影响）。但秤仍钝（hit@3 差异只是 18 条里少中 2 条），**改 `WEIGHT_BONUS_SCALE_API` 仍须用户拍板**。
  → 快照约定：`retrieval_candidates_v1.jsonl` 是 2026-07-19 冻结基线（`test_retrieval_goldset` 硬断言依赖它，**不要改**）；`v2` 是同一批候选、只按新档位重算 `journal_weight` 的版本，供对比用（`retrieval_calibrate.py --candidates`）。
  → **真正的短板不是查询条数，是标注深度**：应用不落盘任何检索历史（`/search`、`/chat` 全不写盘，前端"最近检索"只有 15 条 localStorage 且 pywebview 隐私模式关窗即毁），所以"等用户自然攒够"永远不会发生。**已于 2026-08-10 改四条研究工作流强制写《检索发现日志》**（查询词逐字、名次、处置、弃用理由、漏召回单列），以后自动攒。
  → **档位已修（2026-08-10，用户拍板）**：① 修 `identify` 的 ISSN 优先 bug（ISSN 命中后并入同刊的刊名目录信号，治好"同刊分裂成两档"）；② `PERSONAL_MAPPING_DEFAULTS` 由个人预设改为跟随引擎原型 `A_法学个人`；③ 新增私有目录 `field_focus`（用户对具体期刊的最终裁定，能升能降，`priorityOrder` 第一位、优先于含 CLSCI 在内的一切名录，level 认正式与娱乐两套档名）。最高两档占比 71.4% → **41.8%**（1430 篇真实题录实测；百分比随题录增删漂移，判断量级用，别当契约）。
  → **新增目录的四处必改点**（漏任一处都静默失效）：`grading_config.json` 的 ① `catalogs` 注册、② `disciplines.<学科>.catalogs`、③ `archetypes.<原型>.map` 里**每个 level 一个 token**、④ **`resolution.priorityOrder`**——最后这条最容易漏，不在列表里的目录**永远选不上主档位**（`resolver.py` 的 priority-max 只遍历 priorityOrder）。另登记 `catalog_registry.py`。
  → **还没做的**：① 待审清单里剩余可疑档位（`Celebrity Studies`、商科刊群、城市地理刊群等）；② `clsci.json` 只有 31 条种子样本，注释自述"请替换为完整名单"；③ 用户本人扩标注（每查询高相关文献从 2 篇提到 5—8 篇 + 池外补标 + 裁决悬置的 C03）——**用户 2026-08-10 表示认可现有标注，暂不逐条重标**；④ 本地与 light 模式无快照采集脚本，无法校准。
  → **动 `WEIGHT_BONUS_SCALE_API` 会撞三处硬断言**：`test_retrieval_goldset.py:60`、`:64-70`、`test_retriever_candidate_loading.py:63-68`。**仍然：不要凭感觉调这个权重。**
- **SAC 补生成（backfill）从未真跑过**：会消耗 API 额度且会改真实库，跑之前必须先问用户。
- **`import_only_pdf` 已改为严格「只导有 PDF」**（1.0.9，用户 2026-07-15 拍板）：不再豁免法源类
  （statute/case/standard/report）。重要法条由用户自己配 PDF 后入库。开关字面意思与行为现在一致。
  （去重的 `_DEDUP_LAW_TYPES` 是**另一回事**：法源同名不同版本不合并，与本项无关，保留。）
- **「清空并从头重建索引」**（1.0.9）：`server._reset_index_full()` + `POST /index/reset`（须 `confirm`）。
  只清索引产物（`INDEX_IN_DATA` + `index_manifest.json`：移到 `<数据家>\_reset_index_backup_<ts>\`；
  向量表走 `drop_table`），**保留** wiki/categories/summaries(SAC)/pagemap/meta/settings/`0_Agent*`。
  依赖：重建时 `index_semantic` 以 `mode=overwrite` 重造表、`load_all→reindex_missing_pages` 回灌 wiki 向量。
  UI 在设置页「🧹 清空并从头重建索引」，两步确认。**改这里前先想清楚 `backup.py` 的 CORE/INDEX 分类。**
...
- **打包发布**：见 [docs/RELEASE.md](docs/RELEASE.md)。发布形态已定案（2026-07-14）：**只发 Inno 安装器**（便携 zip 已砍）、**用户级安装**（不弹 UAC，可改装到 D 盘）、**数据与程序同目录**（索引/模型/wiki/`0_Agent*` 全在安装目录内，一个文件夹搬走）、数据目录由 `LocalKB` **改名为 `PaperPiggy`**（环境变量 `LOCALKB_*` 保持不变）。
  ...
  **发布记录（别在这里再抄当前版本号——会漂）**：当前版本以 `config.APP_VERSION` 为唯一事实源，**逐版本变更看 [CHANGELOG.md](CHANGELOG.md)**（从 v1.0.1 一路记到现在）。里程碑仅供定位：v1.0.0 首发带启动 bug（VBS SW_HIDE）已删；v1.0.1 起持续迭代（备份 / 应用内一键升级 / 一批深索·扫描件修复 / 只导 PDF 严格化 / 清空重建 / 顶栏更新徽标 / macOS 源码适配）。出包链路：`build_bundle.py` → `installer/build_installer.py` → `gh release create v<版本> dist-installer\<显式三文件>`（**别用 `dist-installer\*`**，会把上一版残留产物也传上去）。
  ⚠️ **发新版后**：CHANGELOG 必更（check_guides 不管散文）；面向用户文案通读清残留（§0.5①）；本文件里凡"当前版本=x.y.z"式的硬编码一律**别写**，指向 config.APP_VERSION / CHANGELOG。
...
| 07-14 | 删分发包、归档、建 git、指引本地化 | git 历史 |
| 07-15 | 发布迭代轮：v1.0.1→v1.0.10 安装器发版 + 应用内一键升级 + 一批深索/扫描件修复 + 只导 PDF 严格化 + 清空重建 + 顶栏更新徽标 + macOS 源码适配 | git 历史 / [CHANGELOG.md](CHANGELOG.md) |
