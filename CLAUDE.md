...
- **`updater.py` 是死代码**（重要，别白读）：全仓 grep 过，`server.py` / `launcher.py` / 前端**没有任何地方调用它**。UI 上那个「自动检查更新」是**知识库增量索引**，跟版本升级无关。v1.0.0 的唯一升级路径 = 重下安装器覆盖安装（走 Inno，不走它）。
  → 后果：Inno 用 `Flags: ignoreversion` **无条件覆盖** `app\` 下所有 `.py`。本项目卖点是「明文 .py，用户可以直接改」，那么**用户改过的代码在升级安装时会静默消失，连 `.bak` 都没有**。updater 里那套「改过的文件先备份」的保护一次都不会执行。README 已如实告知用户先备份。
  → 接线之前必须先修两个坑（已写在 `updater.py` 文件头）：① 回滚用 `rmtree(ignore_errors=True)` + `move`，删不干净时会把备份塞进残留目录变成 `app\app.bak-x.y.z\`，而函数照样返回「回滚成功」——**假回滚**；② 程序目录可写性未预检。
- **打包发布**：见 [docs/RELEASE.md](docs/RELEASE.md)。发布形态已定案（2026-07-14）：**只发 Inno 安装器**（便携 zip 已砍）、**用户级安装**（不弹 UAC，可改装到 D 盘）、**数据与程序同目录**（索引/模型/wiki/`0_Agent*` 全在安装目录内，一个文件夹搬走）、数据目录由 `LocalKB` **改名为 `PaperPiggy`**（环境变量 `LOCALKB_*` 保持不变）。
  ...
  **仍卡在需要用户本人做的两件事**：① 建 GitHub 仓库 `DrinkTea905/paper-piggy`（公开）② 把模型传到 Release `models-v1` tag。
