...
**LocalKB（产品名「论文猪」）** —— Windows 本地知识库桌面应用，面向**法学 / 社科研究者**。
...
> **旧铁律（已作废）**：~~先备份 → 只改 LocalKB源码 → sync_app.ps1 同步到 LocalKB\app → 在包里验证~~
...
> ⚠️ **`LOCALKB_MODELS` 必须显式设**。`config.py` 的 `_resolve_models()` 已经不再兜底任何开发机的绝对路径了，不设就会指向一个不存在的 `LocalKB源码\models`，本地嵌入/重排静默失效。

**④ 只有真要出包时才构建**（不再有常驻的测试包）

```powershell
& build\py312\python.exe LocalKB源码\build_bundle.py    # 产物在 LocalKB源码\dist\
```
...
一旦仓库根出现 `python\python.exe`，开发态就会被误判成 bundle，`DATA` 改指 `%LOCALAPPDATA%\LocalKB`，**你现有的 `LocalKB源码\data` 开发索引就被抛弃了**。
