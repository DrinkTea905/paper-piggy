...
少年司法论文初稿按选型决策树选出候选结构原型、再逐个做准入自检，默认先交两张骨架卡并推荐一条，用户选定前不得起草全文；只有用户已经明确选路且明确不要比较时才可跳过。**题目套不进七类原型时自建骨架（手册卡⑧），与选用原型平权，不得硬套**——七类归纳自陈瑞华一个作者、一个部门法的 36 篇，样本内部就有归不进去的。出卡之前必须先做**选型前侦查**（候选池题录 20—40 条，从中挑 6—10 篇核心：完整精读 2—3 篇、其余只读标题树＋引言结论；读满仍定不下就停下来问用户。只回答选型七问，产出侦查表与两卡一起交用户）——**侦查的局部阅读不得落成引注**，正文引用一律走证据底座的完整精读。骨架卡除逐字标题树外还必须交代**每章每节准备写什么**（回答哪个问题｜用哪类材料｜得出什么判断）。四条研究工作流均在开工前问一次是否派子代理、派在哪些环节，由用户决定，回答前不派。通用初稿必须提示“尚未经过其他部门法训练验证”。旧 `写论文与综述.md` 迁移时，历史出厂原文可升级到 `综述.md`，用户改过的文件必须改名保留，绝不删除或覆盖。
