...
**[⬇️ 下载最新版](https://github.com/DrinkTea905/paper-piggy/releases/latest)**

| | 大小 | 说明 |
|---|---|---|
| **PaperPiggy-x.y.z-win64.exe** | 177 MB | **安装器（推荐）**。首次启动时按需下载嵌入模型（约 900 MB）；选 API 模式则无需下载。 |
| **PaperPiggy-x.y.z-portable.zip** | 282 MB | 便携版，解压即用，数据存在软件目录内（适合 U 盘）。 |

系统要求：Windows 10/11 64 位。无需预装 Python，无需 VC++ 运行库（都在包里）。缺 WebView2 时安装器会自动装。
